...
A lot of the more recent text on this blog is written with the help of an AI assistant. I manually edit the output and still toil away on it b/c it feels crappy to put something out there for people to read when it's been extruded from a machine. It's more obvious with processed foods. When I choose a can of Pringles, I know what I'm doing. With text, it's not so clear.

I have been transparent from the start with the onetimesecret repo, including an AI notice in the main README.md. I'm transparent here with the god awful AI-generated SVGs and notes in posts where AI was used. But more than transparency, I want to keep track of what I wrote and what an AI wrote *during the process of writing it*. So over the weekend I got to hacking something together, heavily inspired by [Nerd Fonts](https://www.nerdfonts.com/) and based on the Nerd Fonts tooling to implement a simple provenance marker in the typeface itself. The idea is to mark the text at the glyph level, so that it travels with the words, surviving copy and pastes, and doesn't depend on just trying to keep track yourself.
...
The fonts on this site are now Nerd Fonts with provenance builds, indicated by "P+" (there's a 32 character limit so I had to keep it short).
...
With Provenance on, the marked line and the paragraph inside the panel show the sawtooth. With P+ off, the same characters render in the plain faces and the marks disappear. The toggle is client-side only and resets on the next page load.

## Under the hood

The mark is not a style or an annotation layered on top of the text. It is a code point sitting in the string after each character. Below is the same word twice, once as typed and once with the mark. The switch never changes what is in either string. It only changes whether the font has a glyph for U+E0101.
